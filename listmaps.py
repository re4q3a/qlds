...
excluded_maps = ['arcanecitadel', 'arkinholm', 'basesiege', 'beyondreality', 'bitterembrace', 'bloodlust', 'campercrossings', 'citycrossings', 'coldcathode', 'coldwar', 'concretepalace', 'corrosion', 'courtyard', 'cursed', 'deepinside', 'delirium', 'demonkeep', 'devilish', 'diesirae', 'dismemberment', 'dividedcrossings', 'divineintermission', 'doubleimpact', 'dreadfulplace', 'duelingkeeps', 'elder', 'electrocution', 'falloutbunker', 'finnegans', 'fluorescent', 'foolishlegacy', 'fuse', 'futurecrossings', 'gospelcrossings', 'gothicrage', 'grimdungeons', 'hektik', 'heroskeep', 'industrialrevolution', 'infinity', 'innersanctums', 'intervention', 'ironworks', 'japanesecastles', 'jumpwerkz', 'leftbehind', 'leviathan', 'limbus', 'lostparadise', 'lostworld', 'mcsarges', 'midlifecrisis', 'namelessplace', 'newcerberon', 'overgrowth', 'overlord', 'phrantic', 'pillbox', 'pulpfriction', 'purgatory', 'qzpractice1', 'qzpractice2', 'qztraining', 'ragnarok', 'railyard', 'realmofsteelrats', 'rebound', 'refinery', 'reflux', 'repent', 'retribution', 'revolver', 'satanic', 'scornforge', 'shiningforces', 'siberia', 'silence', 'sinister', 'skyward', 'solarium', 'solid', 'somewhatdamaged', 'sorrow', 'spacechamber', 'spacectf', 'spidercrossings', 'stonekeep', 'stronghold', 'terminalheights', 'terminatria', 'terminus', 'thedukesgarden', 'theepicenter', 'theoldendomain', 'threestory', 'tornado', 'toxicity', 'troubledwaters', 'useandabuse', 'verticalvengeance', 'vortexportal', 'warehouse', 'wargrounds', 'wicked', 'windowpain', 'windsongkeep', 'zen', 'ospctf1', 'ospctf2', 'aerowalk', 'almostlost', 'asylum', 'battleforged', 'blackcathedral', 'bloodrun', 'brimstoneabbey', 'campgrounds', 'cannedheat', 'castledeathstalker', 'chemicalreaction', 'cliffside', 'cure', 'deadandgone', 'deathorglory', 'dredwerkz', 'drunkenmummy', 'electrichead', 'eviscerated', 'evolution', 'eyetoeye', 'fatalinstinct', 'furiousheights', 'hearth', 'hellsgate', 'henhouse', 'hiddenfortress', 'houseofdecay', 'longestyard', 'seamsandbolts', 'servitude', 'shakennotstirred', 'smash', 'spillway', 'theatreofpain', 'theedge', 'thunderstruck', 'quarantine', 'monastery', 'overkill', 'trinity']
...
class listmaps(minqlx.Plugin):
    ...
    def list_map_names(self, player, msg, channel):
        ...
        try:
            ...
            # REMOVE EXISTIN MAPS FROM THE LIST:
            # rename 'lines' to 'all_lines', we will remove default maps from this list
            # and then copy the modified content to the final 'lines' list.
            all_lines = maps_file.readlines()
            maps_file.close()
        except IOError:
            channel.reply("^4Server^7: Map List creation ^1failed^7. Contact a server admin.")
            return

        # REMOVE EXISTING MAPS FROM THE LIST
        # 'lines' will not contain any default maps
        lines = [i for i in all_lines if i.strip() not in excluded_maps]
