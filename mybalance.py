...
DEFAULT_LAST_ACTION = "ignore" # re4: changed to ignore so uneventeams.py takes over
        ...
        while abs(red_min_blue(excluded_teams)) >= 1:
            ...
            if self.is_even(diff): # one team has an even amount of people more than the other
                ...
                if self.prevent or self.last_action == "ignore":
                    ...
                    #self.msg("^6Uneven teams^7: {} will not be moved to spec".format(last.name)) # re4: commented so uneventeams.py takes over
    ...
    def handle_round_count(self, round_number):
        ...
        if abs(diff) >= 1:
            ...
            if not last:
                ...
                #self.msg("^6Uneven teams detected!^7 At round start i'll move {} to {}".format(n, to)) # re4: commented to not receive message when teams are uneven, uneventeams.py takes over
            else:
                m = 'lowest player' if n == 1 else '{} lowest players'.format(n)
                m = " and move the {} to {}".format(m, to) if n else ''
                #self.msg("^6Uneven teams detected!^7 Server will auto spec {}{}.".format(last.name, m)) # re4: commented to not receive message when teams are uneven, uneventeams.py takes over
