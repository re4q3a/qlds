...
if ( __name__ == '__main__' ):
    ...
    try:
        ...
        if ( args.password is not None ):
            ...
            socket.plain_username = 'rcon'.encode()
            socket.plain_password = args.password.encode()
            socket.zap_domain = 'rcon'.encode()
        socket.setsockopt_string( zmq.IDENTITY, args.identity )
        socket.connect( args.host.encode() )
        ...
        while ( True ):
            ...
            while ( True ):
                try:
                    ...
                    WriteMessageFormatted( msg.decode() )
    except Exception as e:
        logging.info( e )
